/**
 * Enhanced Bulletproof Speech Recognition System
 * Optimized for offline-first operation with graceful network error handling
 */
...
export class BulletproofSpeechRecognition {
  ...
  private async tryNativeRecognition(): Promise<boolean> {
    ...
    return new Promise((resolve) => {
      try {
        ...
        // Optimized configuration for continuous recognition
        recognition.continuous = true;
        recognition.interimResults = true;
        recognition.lang = 'en-US';
        recognition.maxAlternatives = 1;

        // Extended timeout for continuous recognition (30 seconds + retry buffer)
        const timeoutDuration = Math.min(30000 + (this.retryCount * 2000), 60000);
        ...
        recognition.onresult = (event: any) => {
          ...
          for (let i = event.resultIndex; i < event.results.length; i++) {
            ...
            // Note: In continuous mode, we don't stop automatically after final results
            // The user will manually stop when they're done speaking
          }
        };
   ...
   */
  private async showTextInputFallback(fieldName: string): Promise<void> {
    return new Promise((resolve) => {
      const dialog = document.createElement('div');
      dialog.style.cssText = `
        position: fixed;
      ...
      content.innerHTML = `
        ...
        <h2 style="margin: 0 0 0.5rem 0; color: #1f2937; font-size: 1.5rem; font-weight: 700;">
          💬 Quick Text Input
        </h2>
        <p style="margin: 0 0 1.5rem 0; color: #6b7280; line-height: 1.5;">
          Voice input isn't available right now. Please type your ${fieldName} here:
        </p>

        <textarea
          id="quick-input-text"
          style="width: 100%; height: 150px; padding: 0.875rem;
                 border: 2px solid #e5e7eb; border-radius: 8px; font-size: 1rem;
                 resize: vertical; font-family: inherit; line-height: 1.5;
                 margin-bottom: 1.5rem; transition: border-color 0.2s, box-shadow 0.2s;"
        ...
        <div style="display: flex; gap: 0.75rem;">
          <button
            id="save-text"
            style="flex: 1; padding: 0.75rem 1.5rem; background: #3b82f6; color: white;
          ...
          <button
            id="cancel-text"
            style="flex: 1; padding: 0.75rem 1.5rem; background: #6b7280; color: white;
                   border: none; border-radius: 8px; font-size: 0.95rem; font-weight: 600;
                   cursor: pointer; transition: background 0.2s;"
          >
            Cancel
      ...
      const saveBtn = content.querySelector('#save-text') as HTMLButtonElement;
      const cancelBtn = content.querySelector('#cancel-text') as HTMLButtonElement;
      const textarea = content.querySelector('#quick-input-text') as HTMLTextAreaElement;
  ...
  async start(fieldName: string): Promise<void> {
    ...
    try {
      ...
      console.log('❌ MediaRecorder failed, using text input fallback');

      // Strategy 3: Text input fallback
      await this.showTextInputFallback(fieldName);
