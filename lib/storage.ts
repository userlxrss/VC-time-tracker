...
export function autoGenerateMonthlySalaries(): void {
  ...
  const employees = [
    {
      ...
      reimbursements: [
        { description: 'Ergonomic Chair', amount: 5611, date: '2025-09-09' },
        { description: 'Standing Desk', amount: 5186, date: '2025-09-09' },
        { description: 'Monitor', amount: 4639, date: '2025-09-17' },
        { description: 'Walking Pad', amount: 3859, date: '2025-11-12' }
      ]
    }
  ...
  employees.forEach(employee => {
    const existingRecords = getSalaryRecordsForEmployee(employee.id);

    // Generate equipment reimbursement records
    if ((employee as any).reimbursements) {
      (employee as any).reimbursements.forEach((reimbursement: any) => {
        const reimbDate = new Date(reimbursement.date);
        const paymentMonth = reimbDate.toLocaleDateString('en-US', { month: 'long', year: 'numeric' });
        const recordId = `reimb_${reimbDate.getTime()}_${employee.id}_${reimbursement.description.replace(/\s/g, '')}`;

        // Check if this reimbursement already exists
        const existingReimbursement = existingRecords.find(record => record.id === recordId);

        if (!existingReimbursement && reimbDate <= today) {
          const reimbursementRecord: SalaryRecord = {
            id: recordId,
            employeeId: employee.id,
            employeeName: employee.name,
            amount: reimbursement.amount,
            currency: 'PHP',
            paymentMonth: `${reimbursement.description}`,
            workPeriodStart: reimbursement.date,
            workPeriodEnd: reimbursement.date,
            status: 'paid',
            paidDate: reimbursement.date,
            paidBy: 1, // Boss ID 1
            autoGenerated: true,
            generatedDate: new Date().toISOString(),
            dueDate: reimbursement.date,
            reminderSent: false,
            employeeNotified: true,
            createdAt: reimbursement.date,
            updatedAt: new Date().toISOString()
          };

          saveSalaryRecord(reimbursementRecord);
        }
      });
    }

    // Generate salary history from start date to current month
    // Payment is on the 25th of each month, covering work from 25th of previous month to 24th of current month
    const startDate = new Date(employee.startDate);
    let currentPaymentDate = new Date(startDate.getFullYear(), startDate.getMonth() + 1, 25, 12); // First payment is next month

    while (currentPaymentDate <= today) {
      const paymentMonth = currentPaymentDate.toLocaleDateString('en-US', { month: 'long', year: 'numeric' });

      // Check if salary already exists for this payment month
      const existingSalary = existingRecords.find(record => record.paymentMonth === paymentMonth);

      if (!existingSalary) {
        // Work period: 25th of previous month to 24th of payment month
        const workPeriodStart = new Date(currentPaymentDate.getFullYear(), currentPaymentDate.getMonth() - 1, 25, 12);
        const workPeriodEnd = new Date(currentPaymentDate.getFullYear(), currentPaymentDate.getMonth(), 24, 12);

        // Status: paid if payment date has passed, pending if upcoming
        const isPaid = currentPaymentDate < today;

        const salaryRecord: SalaryRecord = {
          id: `sal_${currentPaymentDate.getTime()}_${employee.id}`,
          employeeId: employee.id,
          employeeName: employee.name,
          ...
          paidDate: isPaid ? currentPaymentDate.toISOString() : null,
          paidBy: isPaid ? 1 : null, // Assume boss ID 1 paid it
          autoGenerated: true,
          generatedDate: new Date().toISOString(),
          dueDate: currentPaymentDate.toISOString().split('T')[0],
          reminderSent: false,
          employeeNotified: isPaid,
      ...
      currentPaymentDate = new Date(currentPaymentDate.getFullYear(), currentPaymentDate.getMonth() + 1, 25, 12);
    }
  });
}
