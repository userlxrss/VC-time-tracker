...
import { Clock, Users, TrendingUp, Calendar, MoreHorizontal, Coffee } from "lucide-react"
...
import { formatHours, formatBreakDuration } from "@/lib/utils"
...
export function EnhancedUserCard({
  ...
  const { isOnBreak, currentActiveBreak, stats } = useBreak()
  const isClockedIn = user.status === 'clocked_in'
  const currentUserOnBreak = isOnBreak && currentActiveBreak

  const getStatusColor = (status: string) => {
    switch (status) {
      case 'clocked_in':
        return currentUserOnBreak ? 'warning' : 'success'
      case 'clocked_out':
        return 'secondary'
      case 'on_break':
        return 'warning'
  ...
  const getStatusText = () => {
    if (currentUserOnBreak) {
      return currentActiveBreak?.type === 'lunch' ? '🍽️ On Lunch' : '☕ On Break'
    }
    switch (user.status) {
      case 'clocked_in':
        return '🟢 Active'
  ...
  const getEffectiveStatus = () => {
    return currentUserOnBreak ? 'on_break' : user.status
  }
  ...
  if (compact) {
    return (
      <>
            ...
            <div className="flex items-center gap-4">
              <div className="relative">
                ...
                <div className="absolute -bottom-1 -right-1">
                  <StatusIndicator status={getEffectiveStatus()} size="sm" />
                </div>
              </div>
                  ...
                  {currentUserOnBreak && (
                    <div className="flex items-center gap-1">
                      ...
                      <span className="text-xs text-orange-500">
                        {currentActiveBreak?.type === 'lunch' ? 'Lunch' : 'Break'}
                      </span>
                    </div>
                  )}
                </div>
              ...
              <div className="flex flex-col gap-2">
                <Badge
                  variant={getStatusColor(getEffectiveStatus())}
                  size="sm"
                ...
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end">
                    ...
                    {isClockedIn && !currentUserOnBreak ? (
        ...
        {/* Break Timer for Compact View */}
        {currentUserOnBreak && (
          <BreakTimer compact className="mt-4" />
        )}
      </>
    )
  }
  ...
  return (
    <>
          ...
          <div className="flex items-start justify-between">
            <div className="flex items-center gap-4">
              <div className="relative">
                ...
                <div className="absolute -bottom-1 -right-1">
                  <StatusIndicator status={getEffectiveStatus()} />
                </div>
              </div>
            ...
            <div className="mb-6 p-3 bg-orange-50 dark:bg-orange-900/20 rounded-lg border border-orange-200 dark:border-orange-800">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-medium">Today's Breaks</span>
                </div>
                <div className="flex items-center gap-3 text-sm">
                  <span>{stats.totalBreaksToday} breaks</span>
                  <span>{formatBreakDuration(stats.totalBreakTimeToday)}</span>
                </div>
              </div>
            </div>
          )}
          ...
          <div className="flex gap-2">
            {isClockedIn && !currentUserOnBreak ? (
              ...
              <Button
                ...
                disabled
              >
                {currentActiveBreak?.type === 'lunch' ? '🍽️ On Lunch' : '☕ On Break'}
              </Button>
            )}
      ...
      {/* Full Break Timer for non-compact view */}
      {currentUserOnBreak && (
        <BreakTimer className="mt-6" />
      )}
    </>
  )
}