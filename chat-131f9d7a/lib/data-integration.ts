...
class DataSyncManager {
  ...
  constructor() {
    ...
    if (typeof window !== 'undefined') {
      ...
      // Set up a custom event system for same-tab communication
      this.setupSameTabEvents();
    }
  }

  private setupSameTabEvents() {
    // Use a custom event target for same-tab communication
    if (typeof window !== 'undefined' && !window.vcDataSyncTarget) {
      window.vcDataSyncTarget = new EventTarget();
    }
  }
  ...
  private handleSameTabEvent(event: CustomEvent<DataSyncEvent>) {
    this.queueEvent(event.detail);
  }

  private queueEvent(event: DataSyncEvent) {
  ...
  private processEventQueue() {
    ...
    // Process events asynchronously to prevent race conditions
    setTimeout(() => {
      ...
      // Continue processing if new events were added
      this.processEventQueue();
    }, 0);
  }
  ...
  public addListener(listener: DataSyncListener) {
    ...
    // Set up same-tab event listener
    if (typeof window !== 'undefined' && window.vcDataSyncTarget) {
      window.vcDataSyncTarget.addEventListener('vcDataSync', this.handleSameTabEvent.bind(this) as EventListener);
    }
  }
  ...
  public broadcastEvent(event: Omit<DataSyncEvent, 'timestamp'>) {
    ...
    if (typeof window !== 'undefined') {
      try {
        ...
        // Handle localStorage quota issues silently
        console.warn('Failed to store sync event in localStorage:', error);
      }

      // Also dispatch a same-tab event for immediate local sync
      if (window.vcDataSyncTarget) {
        window.vcDataSyncTarget.dispatchEvent(new CustomEvent('vcDataSync', { detail: syncEvent }));
      }
    }
  }
}
