...
export function useTimeTracking(userId: number): TimeTrackingState & TimeTrackingActions {
  const [state, setState] = useState<TimeTrackingState>(() => {
    ...
    const allTimeEntries = getTimeEntriesForUser(userId);
...
export interface SalaryManagementActions {
  ...
  confirmSalaryPayment: (salaryId: string) => SalaryPaymentResult; // Alias for confirmReceipt
...
export function useSalaryManagement(
  ...
  // Load salary data when component mounts
  useEffect(() => {
    const loadSalaryData = async () => {
      setState(prev => ({ ...prev, isProcessing: true }));
      try {
        const [currentMonth, history, pending] = await Promise.all([
          getCurrentMonthSalary(employeeId),
  ...
  const refreshSalaryData = useCallback(async () => {
    ...
    try {
      ...
      console.error('Failed to refresh salary data:', error);
      setState(prev => ({ ...prev, isProcessing: false }));
    }
  }, [employeeId]);
  ...
  return {
    ...
    markAsPaid,
    confirmReceipt,
    confirmSalaryPayment: confirmReceipt, // Alias for confirmReceipt
    refreshSalaryData
  };
}
...
// ==================== CENTRALIZED DATA ACCESS HOOK ====================

export interface CentralizedDataState {
  getTimeEntriesForUser: (userId: number) => TimeEntry[];
  getLeaveRequestsForUser: (userId: number) => LeaveRequest[];
  getSalaryPaymentsForUser: (userId: number) => SalaryPayment[];
  getAllTimeEntries: () => TimeEntry[];
  getAllLeaveRequests: () => LeaveRequest[];
  refreshData: () => void;
}

export function useCentralizedData(): CentralizedDataState {
  ...
  const getTimeEntriesForUserConsistent = useCallback((userId: number): TimeEntry[] => {
    return getTimeEntriesForUser(userId);
  }, []);

  const getLeaveRequestsForUserConsistent = useCallback((userId: number): LeaveRequest[] => {
    return getLeaveRequestsForUser(userId);
  }, []);

  const getSalaryPaymentsForUserConsistent = useCallback((userId: number): SalaryPayment[] => {
    return getSalaryPaymentsForUser(userId);
  }, []);

  const getAllTimeEntriesConsistent = useCallback((): TimeEntry[] => {
    return getTimeEntries();
  }, []);

  const getAllLeaveRequestsConsistent = useCallback((): LeaveRequest[] => {
  ...
  return {
    getTimeEntriesForUser: getTimeEntriesForUserConsistent,
    getLeaveRequestsForUser: getLeaveRequestsForUserConsistent,
    getSalaryPaymentsForUser: getSalaryPaymentsForUserConsistent,
    getAllTimeEntries: getAllTimeEntriesConsistent,
    getAllLeaveRequests: getAllLeaveRequestsConsistent,
    refreshData
  };
}
...
async function getSalaryHistory(employeeId: number): Promise<any[]> {
  const allSalaries = await getSalaryRecordsForEmployee(employeeId);
  const currentSalary = await getCurrentMonthSalary(employeeId);
...
async function getPendingSalaries(): Promise<any[]> {
  const { USERS } = require('./constants');
  const allRecords = [];

  for (const user of USERS) {
    const userRecords = await getSalaryRecordsForEmployee(user.id);
    allRecords.push(...userRecords.filter(record => record.status === 'pending'));
  }
