...
import { debugTableStructure } from '@/lib/debug-table';
import {
  handleClockIn,
  handleClockOut,
  handleStartBreak,
...
export default function UserDetailPage() {
  ...
  // Load ALL salary records from Supabase
  useEffect(() => {
    const loadAllSalaryRecords = async () => {
      try {
        const records = await getSalaryRecords();
        console.log('📊 Loaded all salary records from Supabase:', records.length, 'records');
        console.log('🔍 Full salary records data:', JSON.stringify(records, null, 2));
        console.log('👤 Filtered for Larina (user_id = 3):', records.filter(r => r.user_id === 3 || r.employee_id === 3));
        setAllSalaryRecords(records);
      } catch (error) {
        console.error('❌ Failed to load salary records:', error);
      }
    };
    loadAllSalaryRecords();
  }, []);
  ...
  useEffect(() => {
    ...
    // DEBUG: Check what's actually in Supabase
    debugTableStructure();
  }, [userId, currentUserId, selectedEmployeeId]);

  // Load salary data asynchronously
  useEffect(() => {
  ...
  const loadSalaryData = async () => {
    ...
    // Note: salaryHistory is now provided by useSalaryManagement hook
  };

  const generateSampleSalaryData = (employeeId: number) => {
    if (typeof window === 'undefined') return;

    // Use localStorage fallback for this synchronous function
    const cached = localStorage.getItem('vc_salary_records');
    const allRecords = cached ? JSON.parse(cached) : [];
    const existingRecords = allRecords.filter(record => record.employeeId === employeeId || record.userId === employeeId);
    ...
    // Save sample records
    const existingAllRecords = getSalaryRecords();
  ...
  const handleSubmitPayment = async () => {
    try {
      ...
      const isRegularSalary = paymentType === 'Regular Salary';
      const requiresWorkPeriod = paymentType === 'Regular Salary';

      if (!selectedEmployee || !paymentType) {
        showToast('Please select employee and payment type', 'error');
        return;
      }
      ...
      // For Regular Salary: require work period dates
      if (requiresWorkPeriod && (!workPeriodStart || !workPeriodEnd)) {
        showToast('Please provide work period dates for salary payments', 'error');
        return;
      }

      // For non-salary payments: require description
      if (!requiresWorkPeriod && !paymentDescription.trim()) {
        showToast('Please provide description for ' + paymentType.toLowerCase(), 'error');
        return;
      }
      ...
      console.log('💰 Creating new payment with type:', paymentType.toLowerCase());
      console.log('📝 Form paymentType:', paymentType);
      ...
      const salaryRecord = {
        ...
        // Store the payment type from the form
        type: paymentType.toLowerCase(),
        // Only include work period for regular salary
        workPeriodStart: requiresWorkPeriod ? workPeriodStart : null,
        workPeriodEnd: requiresWorkPeriod ? workPeriodEnd : null,
        // Always include description
        description: paymentDescription.trim() || paymentType,
  ...
  const handleSalaryConfirmationForEmployee = async (paymentId: string) => {
    try {
      ...
      if (payment) {
        ...
        // Refresh the allSalaryRecords to update UI
        const records = await getSalaryRecords();
        setAllSalaryRecords(records);
  ...
  const getAllSalaryRecords = () => {
    ...
    return employees.map(employee => {
      ...
      const cached = localStorage.getItem('vc_salary_records');
      const allRecords = cached ? JSON.parse(cached) : [];
      const allSalaries = allRecords.filter(record => record.employeeId === employee.id || record.userId === employee.id);
            ...
            {isBoss ? (
              <div className="space-y-3">
                ...
                <div className="bg-white rounded-lg border border-gray-200 p-4">
                  ...
                  {(() => {
                    // Use ALL salary records from Supabase and filter for pending ones
                    console.log('🔍 All salary records:', JSON.stringify(allSalaryRecords, null, 2));
console.log('💼 Boss view isBoss:', isBoss);
                    const bossPendingSalaries = allSalaryRecords ? allSalaryRecords.filter(payment =>
                      payment.status === 'pending' || payment.status === 'created' || !payment.status || payment.status === 'unpaid'
                    ) : [];
                    console.log('👤 Filtered boss pending salaries:', bossPendingSalaries);

                    if (bossPendingSalaries.length === 0) {
                      return (
                        <div className="text-center py-8 text-gray-500">
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                      ...
                      <tbody className="divide-y divide-gray-100">
                        {(allSalaryRecords || [])
                        .filter(salary =>
                          // For boss: show ALL PAID records from all employees
                          (salary.status === 'paid' || salary.confirmed_by_employee === true || salary.confirmedByEmployee === true)
                        )
                          .sort((a, b) => new Date(b.paid_date || b.paidDate || b.createdAt || '').getTime() - new Date(a.paid_date || a.paidDate || a.createdAt || '').getTime())
                          .map((salary, index) => {
                            const paymentType = salary.type?.toLowerCase() || 'other';
                            const isReimbursement = paymentType === 'reimbursement';
                            console.log('🏷️ Displaying payment:', paymentType, 'for', salary.description);
                            console.log('💾 Raw salary.type:', salary.type);
                            console.log('🔍 isReimbursement result:', isReimbursement);
                            return (
                              <tr key={salary.id || index} className="hover:bg-gray-50 transition-colors">
                                <td className="py-2 px-3">
                                  <span className={`inline-flex px-2 py-0.5 rounded-full text-[10px] font-medium ${
                                    isReimbursement
                                      ? 'bg-blue-100 text-blue-700'
                                    ...
                                    {isReimbursement ? '🏢 Reimburse' :
                                     paymentType === 'salary' ? '💰 Salary' :
                                     paymentType === 'bonus' ? '🎁 Bonus' :
                                     paymentType === 'allowance' ? '💵 Allowance' :
                                     '📋 Other'}
                                  </span>
                                </td>
                                <td className="py-2 px-3 text-xs font-semibold text-gray-800">
                                  {salary.description || 'Salary payment'}
                                  {salary.employee_id && (
                                    <span className="text-gray-500 ml-2">({USERS.find(u => u.id === salary.employee_id)?.firstName || 'Unknown'})</span>
                                  )}
                                </td>
                                <td className="py-2 px-3 text-xs text-gray-600">
                                  {isReimbursement ? '-' : `${formatDate(salary.workPeriodStart || salary.work_period_start || '')} - ${formatDate(salary.workPeriodEnd || salary.work_period_end || '')}`}
                                </td>
                                <td className="py-2 px-3 text-xs font-bold text-green-700">{formatCurrency(salary.amount)}</td>
                                <td className="py-2 px-3 text-xs text-gray-600">
                                  {salary.paid_date || salary.paidDate ? formatDate(salary.paid_date || salary.paidDate) : 'Not paid'}
                                </td>
                              </tr>
                            );
                          })}
                        {(allSalaryRecords || []).filter(salary =>
                          (salary.status === 'paid' || salary.confirmed_by_employee === true || salary.confirmedByEmployee === true)
                        ).length === 0 && (
                          <tr>
                            <td colSpan={5} className="text-center py-8 text-gray-500">
                              <p className="text-sm font-medium">No salary history found</p>
                              <p className="text-xs mt-1">Confirmed payments will appear here</p>
                            </td>
                          </tr>
                        )}
                      </tbody>
              ...
              <div className="space-y-3">
                ...
                {(() => {
                  // Use ALL salary records from Supabase and filter for current employee's pending payments
                  console.log('👤 Employee view pending for userId:', userId);
                  const employeePendingPayments = allSalaryRecords ? allSalaryRecords.filter(payment => {
                    // Filter for current employee's payments that are pending
                    return (payment.employee_id === userId || payment.userId === userId) &&
                           (payment.status === 'pending' || payment.status === 'created' || !payment.status || payment.status === 'unpaid');
                  }) : [];
                  return (
                    <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
                      <div className="p-4 border-b border-gray-200">
                        <h2 className="text-sm font-bold text-gray-800">💰 Pending Confirmations</h2>
                      </div>
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                      ...
                      <tbody className="divide-y divide-gray-100">
                        ...
                        .filter(salary =>
                          // For boss: show ALL PAID records from all employees
                          isBoss ? (salary.status === 'paid' || salary.confirmed_by_employee === true || salary.confirmedByEmployee === true)
                          // For employee: show only their PAID records
                          : (salary.employee_id === userId || salary.userId === userId) && (salary.status === 'paid' || salary.confirmed_by_employee === true || salary.confirmedByEmployee === true)
                        )
                          .sort((a, b) => new Date(b.paid_date || b.paidDate || b.createdAt || '').getTime() - new Date(a.paid_date || a.paidDate || a.createdAt || '').getTime())
                          .map((salary, index) => {
                            const paymentType = salary.type?.toLowerCase() || 'other';
                            ...
                            return (
                              <tr key={salary.id || index} className="hover:bg-gray-50 transition-colors">
                                ...
                                <td className="py-2 px-3 text-xs font-bold text-green-700">{formatCurrency(salary.amount)}</td>
                                <td className="py-2 px-3 text-xs text-gray-600">
                                  {salary.paidDate || salary.paid_date ? formatDate(salary.paidDate || salary.paid_date) : 'Not paid'}
                                </td>
                              </tr>
                            );
                          })}
                        {!(allSalaryRecords || [])
                        .filter(salary =>
                          // Show PAID records for current employee (or all records for bosses)
                          (isBoss ? true : (salary.employee_id === userId || salary.userId === userId)) &&
                          (salary.status === 'paid' || salary.confirmed_by_employee === true || salary.confirmedByEmployee === true)
                        ).length && (
                          <tr>
                            <td colSpan={5} className="text-center py-8 text-gray-500">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="space-y-2 max-h-96 overflow-y-auto">
                  {(() => {
                    const currentPendingSalaries = pendingSalaries ? pendingSalaries.filter(payment =>
                      payment.status === 'pending'
                    ) : [];

                    if (currentPendingSalaries.length === 0) {
                      return (
                        <div className="text-center py-8 text-gray-500">
                    ...
                    return currentPendingSalaries.map((payment) => (
